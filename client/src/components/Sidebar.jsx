...
const Sidebar = ({ onClose }) => {
  ...
  return (
    <aside className="w-72 h-screen flex flex-col p-6 z-20 border-r border-white/10 bg-black text-white">
      <div className="flex items-center justify-between gap-3 mb-10 px-2">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-lg bg-gradient-to-br from-white to-zinc-500 flex items-center justify-center shadow-lg shadow-white/10">
            <Code2 size={24} className="text-black" />
      ...
      <div className="pt-6 border-t border-white/10">
        <button
          ...
          className="flex items-center gap-3 px-4 py-3 rounded-xl w-full text-slate-400 hover:text-white hover:bg-white/5 transition-all group"
        >
          <LogOut size={20} className="group-hover:text-red-400 transition-colors" />
          <span className="font-medium">Logout</span>
        </button>
